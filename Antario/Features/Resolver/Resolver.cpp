...
void Resolver::AnimationFix(C_BaseEntity* pEnt)
{
	...
	float* PosParams = (float*)((uintptr_t)pEnt + netvar::data[ hash::CompileTime( "CBaseAnimating->m_flPoseParameter") ]);
	bool update = false;
	bool shot = false;
	...
	if (storedSimtime[pEnt->EntIndex()] != pEnt->GetSimulationTime())
	{
		...
		memcpy(StoredLayers[pEnt->EntIndex()], pEnt->AnimOverlays(), (sizeof(AnimationLayer) * pEnt->GetAnimLayerCount()));
	...
	memcpy(pEnt->AnimOverlays(), StoredLayers[pEnt->EntIndex()], (sizeof(AnimationLayer) * pEnt->GetAnimLayerCount()));
...
void Resolver::FrameStage(ClientFrameStage_t stage)
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		if ( isTeamMate || pPlayerEntity == g::pLocalEntity )
			continue;
